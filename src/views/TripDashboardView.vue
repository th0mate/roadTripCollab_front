...
const getStopIconClass = (t: string) => ({
...
const getCategoryIconClass = (c: string) => ({
...
const budgetStats = computed(() => {
  ...
  return {
    ...
    percentage: total > 0 ? Math.min(100, (spent / total) * 100) : 0,
    perUser
  };
});
...
const initializeManualShares = () => {
  ...
  trip.value.participants.forEach((p: any) => {
    manualShares.value[p.id] = Number(share.toFixed(2));
  });
};

watch(showSettlementModal, (val) => {
  if (val && Object.keys(manualShares.value).length === 0) initializeManualShares();
});
...
const calculateItineraryByDay = async () => {
  ...
  for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
    ...
    if (morningAcc) {
      ...
      dayIt.push({
        ...morningAcc,
        id: `start-${morningAcc.id}-${dateStr}`,
        displayTitle: `Départ : ${morningAcc.title}`,
        isAccommodationHub: true,
        isMorningDeparture: true,
        latitude: parseFloat(morningAcc.latitude),
        longitude: parseFloat(morningAcc.longitude),
        arrivalDate: `${dateStr}T${t}:00`,
        departureDate: `${dateStr}T${t}:00`
      });
    } else if (dateStr === extractDateLocal(trip.value.startDate)) {
      dayIt.push({
        id: `start-trip-${dateStr}`,
        ...
        isAccommodationHub: true,
        isMorningDeparture: true,
        arrivalDate: `${dateStr}T${tripSettings[dateStr]?.startTime || '09:00'}:00`,
        departureDate: `${dateStr}T${tripSettings[dateStr]?.startTime || '09:00'}:00`
      });
    }

    const activitiesOnly = arrivals.filter((s: any) => s.type !== 'city');
    activitiesOnly.sort((a: any, b: any) => (extractTimeLocal(a.arrivalDate) || '00:00').localeCompare(extractTimeLocal(b.arrivalDate) || '00:00') || (a.order - b.order)).forEach((a: any) => dayIt.push({
      ...a,
    ...
    if (eveningAcc && extractDateLocal(eveningAcc.arrivalDate!) < dateStr) {
      dayIt.push({
        ...eveningAcc,
        id: `end-${eveningAcc.id}-${dateStr}`,
        displayTitle: `${eveningAcc.title} (Retour)`,
        isAccommodationHub: true,
        isEveningReturn: true,
        latitude: parseFloat(eveningAcc.latitude),
        longitude: parseFloat(eveningAcc.longitude),
        arrivalDate: null,
        departureDate: null
      });
    }
    ...
    if (dayIt.length === 0) {
      ...
      if (cityStop) dayIt.push({
        ...cityStop,
        latitude: parseFloat(cityStop.latitude),
        longitude: parseFloat(cityStop.longitude)
      });
    }
  ...
  for (const day of days) {
    ...
    for (let i = 0; i < valid.length - 1; i++) {
      const rd = await getGoogleRoute({lat: valid[i].latitude, lng: valid[i].longitude}, {
        lat: valid[i + 1].latitude,
        lng: valid[i + 1].longitude
      });
      if (rd) {
  ...
  for (let i = 0; i < days.length - 1; i++) {
    ...
    if (lastOfCurrent && firstOfNext) {
      ...
      const rd = await getGoogleRoute({
        lat: lastOfCurrent.latitude,
        lng: lastOfCurrent.longitude
      }, {lat: firstOfNext.latitude, lng: firstOfNext.longitude});
      if (rd) {
  ...
  if (lastPoint && !isNaN(parseFloat(startLoc.latitude)) && !isNaN(parseFloat(startLoc.longitude))) {
    if (parseFloat(lastPoint.latitude).toFixed(5) !== parseFloat(startLoc.latitude).toFixed(5) ||
      ...
      }, {lat: parseFloat(startLoc.latitude), lng: parseFloat(startLoc.longitude)});
      if (rd) {
    ...
    day.activities.forEach((act: any, idx: number) => {
      if (act.polylineNext) {
        ...
        if (isReturnRoute) {
          ...
          if (routeFilters.value.activity) drawEncodedRoute(act.polylineNext, {
            color: '#ff00ff',
            weight: 5,
            opacity: 0.9
          });
      ...
      if (act.interDayPolyline && routeFilters.value.main) drawEncodedRoute(act.interDayPolyline, {
        color: '#ccff00',
        weight: 5,
        opacity: 0.9
      });
...
const acceptInvitation = async () => {
  ...
  isSubmitting.value = true;
  try {
    await api.post(`/invitations/${piv.id}/accept`);
    await fetchTripData();
...
const openDeleteModal = (type: 'expense' | 'stop' | 'participant', item: any) => {
  ...
  itemToDelete.value = {
    type,
    id: item.id,
    name: type === 'expense' ? item.title : item.title || item.fullName,
    extraId: item.id,
    action
  };
...
const openHubModal = (activity: any, date: string) => {
  if (!activity.isMorningDeparture) return;
  editingHubDay.value = date;
  hubStartTime.value = extractTimeLocal(activity.departureDate) || '09:00';
...
<template>
  <div class="flex flex-col flex-grow overflow-hidden pt-14">
    <div v-if="loading" class="flex flex-col flex-grow overflow-hidden bg-zinc-50 dark:bg-[#0c0c0e] animate-pulse">
      <div
          class="shrink-0 flex items-center justify-between px-4 h-[52px] bg-white dark:bg-zinc-900/80 border-b border-zinc-200 dark:border-zinc-800/50">
      ...
      <div class="flex-grow flex overflow-hidden" style="height: calc(100vh - 104px);">
        <aside class="w-[340px] shrink-0 flex flex-col p-3 gap-3">
          <div
              class="p-4 bg-white dark:bg-zinc-900 rounded-[1.5rem] border border-zinc-200 dark:border-white/5 space-y-3">
          ...
          <div
              class="flex-grow bg-white dark:bg-zinc-900 rounded-[1.5rem] border border-zinc-200 dark:border-white/5 overflow-hidden flex flex-col">
            ...
            <div class="flex-grow px-3 py-3 space-y-2">
              <div v-for="i in 5" :key="i"
                   class="flex items-center gap-2.5 p-2.5 rounded-xl border border-zinc-100 dark:border-zinc-800/30">
        ...
        <div class="flex-grow rounded-[2.5rem] my-3 mr-3 bg-zinc-200 dark:bg-zinc-800 overflow-hidden relative">
          <div
              class="absolute inset-0 bg-gradient-to-br from-zinc-300/50 to-zinc-200/20 dark:from-zinc-700/50 dark:to-zinc-800/20"></div>
          ...
          <div
              class="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 p-1.5 bg-white/70 dark:bg-zinc-900/80 rounded-2xl">
    ...
    <div v-else-if="trip"
         class="flex flex-col bg-zinc-50 dark:bg-[#0c0c0e] text-zinc-900 dark:text-zinc-100 overflow-hidden flex-grow">
      <div data-onboarding="dash-header"
           class="h-12 shrink-0 bg-white dark:bg-[#161618] border-b border-zinc-200 dark:border-zinc-800/50 flex items-center justify-between px-6 z-40">
        <div class="flex items-center gap-4">
          ...
          <div data-onboarding="dash-participants" class="flex -space-x-3">
            <button v-for="p in trip.participants.slice(0, 5)" :key="p.id"
              ...
              <div v-else
                   class="w-full h-full flex items-center justify-center text-[10px] font-black text-zinc-500 dark:text-zinc-400">
          ...
          <span v-else-if="trip.isPublic"
                class="ml-2 px-3 py-1.5 rounded-xl bg-primary-400/10 text-primary-500 text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 border border-primary-400/20">
      ...
      <div v-if="isInvitationPending"
           class="shrink-0 px-6 py-3 bg-amber-500/10 border-b border-amber-500/30 flex items-center justify-between">
      ...
      <div class="flex-grow flex overflow-hidden" style="height: calc(100vh - 104px);">
        <aside
            class="w-[340px] shrink-0 flex flex-col bg-zinc-50 dark:bg-[#0c0c0e] overflow-hidden z-30 transition-all duration-300"
            :class="{'!w-0 opacity-0 pointer-events-none': showItineraryDrawer && showBudgetDrawer}">
          <section data-onboarding="dash-budget"
                   class="m-3 p-4 bg-white dark:bg-zinc-900 rounded-[1.5rem] border border-zinc-200 dark:border-white/5 relative overflow-hidden shrink-0">
            <div
                class="absolute -top-10 -right-10 w-40 h-40 bg-primary-400/5 rounded-full blur-3xl pointer-events-none"></div>
            <div class="flex items-center justify-between mb-3 relative">
              <div>
                <p class="text-[8px] font-black uppercase tracking-[0.2em] text-zinc-500 dark:text-zinc-400 mb-0.5">
                  Budget</p>
              ...
              <div class="flex gap-1.5">
                <button @click="showExpenseModal = true"
                        class="w-7 h-7 rounded-lg bg-zinc-100 dark:bg-white/5 border border-zinc-200 dark:border-white/10 flex items-center justify-center text-zinc-500 dark:text-zinc-400 hover:bg-primary-400 hover:text-zinc-900 transition-all cursor-pointer">
                  <i class="fi fi-rr-plus text-xs"></i></button>
                <button @click="showBudgetDrawer = true"
                        class="w-7 h-7 rounded-lg bg-zinc-100 dark:bg-white/5 border border-zinc-200 dark:border-white/10 flex items-center justify-center text-zinc-500 dark:text-zinc-400 hover:text-primary-400 transition-all cursor-pointer">
                  <i class="fi fi-rr-wallet text-xs"></i></button>
              </div>
            </div>
            <div class="h-1.5 w-full bg-zinc-100 dark:bg-white/5 rounded-full overflow-hidden mb-3">
              <div class="h-full rounded-full transition-all duration-1000"
                   :class="budgetStats.percentage > 90 ? 'bg-rose-500' : budgetStats.percentage > 70 ? 'bg-amber-400' : 'bg-primary-400 shadow-[0_0_8px_#ccff00]'"
                   :style="{ width: `${budgetStats.percentage}%` }"></div>
            </div>
            <div class="grid grid-cols-2 gap-2">
              <div
                  class="flex items-center gap-2 p-2 bg-zinc-50 dark:bg-white/5 rounded-xl border border-zinc-100 dark:border-white/5">
                <i class="fi fi-rr-gas-pump text-[10px] text-zinc-500 dark:text-zinc-400"></i>
                <p class="text-[10px] font-black text-zinc-900 dark:text-zinc-100">{{
                    Math.round(estimatedFuelCost)
                  }}€</p></div>
              <div
                  class="flex items-center gap-2 p-2 bg-zinc-50 dark:bg-white/5 rounded-xl border border-zinc-100 dark:border-white/5">
                <i class="fi fi-rr-road text-[10px] text-zinc-500 dark:text-zinc-400"></i>
                <p class="text-[10px] font-black text-zinc-900 dark:text-zinc-100">{{
                    Math.round(estimatedTollCost)
                  }}€</p></div>
            </div>
          </section>

          <section
              class="mx-3 mb-3 flex-grow flex flex-col bg-white dark:bg-zinc-900 rounded-[1.5rem] border border-zinc-200 dark:border-white/5 overflow-hidden min-h-0 relative">
            <div
                class="absolute -top-10 -left-10 w-40 h-40 bg-primary-400/5 rounded-full blur-3xl pointer-events-none"></div>
            <div
                class="px-4 py-3 flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800/50 shrink-0 relative">
              <h3 class="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-500 dark:text-zinc-400">
                Itinéraire</h3>
              <button @click="showRouteSettingsModal = true"
                      class="w-6 h-6 rounded-lg flex items-center justify-center text-zinc-400 dark:text-zinc-600 hover:text-primary-400 transition-all cursor-pointer">
                <i class="fi fi-rr-settings text-xs"></i></button>
            </div>
            <div data-onboarding="dash-itinerary"
                 class="px-4 py-2 flex items-center gap-2 border-b border-zinc-100 dark:border-zinc-800/30 shrink-0">
              <button @click="currentDayIndex = Math.max(0, currentDayIndex - 1)" :disabled="currentDayIndex === 0"
                      class="w-6 h-6 rounded-lg bg-zinc-100 dark:bg-white/5 border border-zinc-200 dark:border-white/10 flex items-center justify-center text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white disabled:opacity-30 transition-all cursor-pointer">
                <i class="fi fi-rr-angle-small-left text-xs"></i></button>
              <div class="flex-grow text-center">
                <p class="text-[10px] font-black text-zinc-900 dark:text-zinc-100">J{{ currentDayIndex + 1 }} <span
                    class="text-zinc-400 dark:text-zinc-600">/ {{ daysData.length }}</span></p>
                <p v-if="currentDay" class="text-[9px] text-primary-400 font-bold">
                  {{ formatDate(currentDay.date) }}
                  <span v-if="getCityNameForDay(currentDay)"
                        class="text-zinc-400 dark:text-zinc-500 ml-1">· {{ getCityNameForDay(currentDay) }}</span>
                </p>
              </div>
              <button @click="currentDayIndex = Math.min(daysData.length - 1, currentDayIndex + 1)"
                      :disabled="currentDayIndex >= daysData.length - 1"
                      class="w-6 h-6 rounded-lg bg-zinc-100 dark:bg-white/5 border border-zinc-200 dark:border-white/10 flex items-center justify-center text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white disabled:opacity-30 transition-all cursor-pointer">
                <i class="fi fi-rr-angle-small-right text-xs"></i></button>
            </div>
            <div class="flex-grow overflow-y-auto px-3 py-3 space-y-1.5 custom-scrollbar min-h-0">
              <div v-if="!currentDay || currentDay.activities.length === 0" class="py-8 text-center">
                ...
                <p class="text-[9px] text-zinc-400 dark:text-zinc-600 uppercase tracking-wider font-black">Journée
                  libre</p>
              </div>
              <template v-if="currentDay" v-for="(act, idx) in currentDay.activities" :key="act.id">
                <div @click="focusStopOnMap(act)"
                     class="p-2.5 bg-zinc-50 dark:bg-white/5 rounded-xl border border-zinc-100 dark:border-white/5 hover:border-primary-400/30 transition-all cursor-pointer group/act flex items-center gap-3"
                     :class="{'border-cyan-500/30 bg-cyan-500/10 dark:bg-cyan-500/10': act.isReturnStep}">
                  <div class="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 transition-all"
                       :class="act.isMorningDeparture ? 'bg-amber-400/10 text-amber-500 dark:text-amber-400' : act.isEveningReturn ? 'bg-violet-500/10 text-violet-500 dark:text-violet-400' : act.isReturnStep ? 'bg-cyan-500/10 text-cyan-500 dark:text-cyan-400' : 'bg-primary-400/10 text-primary-500 dark:text-primary-400 group-hover/act:bg-primary-400 group-hover/act:text-zinc-900'">
                    <i :class="getStopIconClass(act.type)" class="text-[10px]"></i></div>
                  <div class="flex-grow min-w-0">
                    <p class="text-[11px] font-black truncate leading-tight text-zinc-900 dark:text-zinc-100">
                      {{ act.displayTitle || act.title }}</p>
                    <p class="text-[9px] text-zinc-500 dark:text-zinc-400 leading-tight">
                      {{ formatTime(act.arrivalDate) || formatStopType(act.type) }}</p>
                  </div>
                  <div class="flex items-center gap-1 opacity-0 group-hover/act:opacity-100 transition-all shrink-0">
                    <button v-if="!act.isReturnStep && !act.isMorningDeparture && !act.isEveningReturn"
                            @click.stop="openEditStopModal(act)"
                            class="w-5 h-5 rounded flex items-center justify-center text-zinc-400 dark:text-zinc-500 hover:text-primary-400 cursor-pointer">
                      <i class="fi fi-rr-edit text-[9px]"></i></button>
                    <button v-if="!act.isAccommodationHub && !act.isReturnStep"
                            @click.stop="openDeleteModal('stop', act)"
                            class="w-5 h-5 rounded flex items-center justify-center text-zinc-400 dark:text-zinc-500 hover:text-rose-400 cursor-pointer">
                      <i class="fi fi-rr-trash text-[9px]"></i></button>
                  </div>
                </div>
                <div
                    v-if="(act.travelTimeToNext || (act.returnRouteData && act.isReturnStep)) && Number(idx) < (currentDay?.activities?.length || 0) - 1"
                    class="flex items-center gap-2 px-2 ml-1">
                  ...
                  <div class="flex flex-wrap items-center gap-2 text-[8px] font-bold text-zinc-400 dark:text-zinc-400">
                    <span>{{
                        formatDuration(act.travelTimeToNext || act.returnRouteData?.duration)
                      }} · {{ act.distanceToNext || act.returnRouteData?.distance }}km</span>
                    <span v-if="act.fuelCostNext"
                          class="flex items-center gap-0.5 text-amber-600 dark:text-amber-500/80">
                      <i class="fi fi-rr-gas-pump"></i>{{ act.fuelCostNext.toFixed(2) }}€
                    </span>
                    <span v-if="act.tollCostNext"
                          class="flex items-center gap-0.5 text-violet-600 dark:text-violet-400/80">
            ...
            <div class="px-3 pb-3 space-y-1.5 shrink-0 border-t border-zinc-100 dark:border-zinc-800/50 pt-2.5">
              <button v-if="!isInvitationPending" @click="onSidebarAddClick"
                      class="w-full py-2 rounded-xl border border-dashed border-zinc-300 dark:border-zinc-800 text-[9px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-600 hover:text-primary-400 hover:border-primary-400/30 transition-all cursor-pointer">
                Ajouter
              </button>
              <button @click="showItineraryDrawer = true"
                      class="w-full py-2 rounded-xl bg-primary-400 text-zinc-900 text-[9px] font-black uppercase tracking-widest hover:bg-primary-500 transition-all cursor-pointer">
                Itinéraire complet
              </button>
            </div>
          </section>
        ...
        <transition name="panel-slide">
          <div v-if="showItineraryDrawer"
               class="w-[380px] shrink-0 flex flex-col bg-white dark:bg-zinc-900 rounded-[1.5rem] border border-zinc-200 dark:border-white/5 m-3 ml-0 overflow-hidden">
            <header
                class="px-4 py-3 border-b border-zinc-100 dark:border-zinc-800/60 flex items-center justify-between shrink-0">
              <div class="flex items-center gap-2.5">
                <div
                    class="w-7 h-7 rounded-lg bg-primary-400/10 flex items-center justify-center text-primary-500 dark:text-primary-400 shrink-0">
                  <i class="fi fi-rr-map-marker text-xs"></i></div>
                <div>
                  <h2 class="text-sm font-black tracking-tight text-zinc-900 dark:text-zinc-100 leading-tight">
                    Itinéraire complet</h2>
                  <p class="text-[9px] text-zinc-400 dark:text-zinc-500 font-bold">
                    {{ formatDateRange(trip.startDate, trip.endDate) }}</p>
                </div>
              </div>
              <button @click="showItineraryDrawer = false"
                      class="w-7 h-7 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center justify-center text-zinc-400 dark:text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 transition-all cursor-pointer shrink-0">
                <i class="fi fi-rr-cross text-xs"></i></button>
            </header>
            <div class="flex-grow overflow-y-auto custom-scrollbar">
              <div v-for="(day, idx) in daysData" :key="day.date">
                <div
                    class="px-4 py-2 bg-zinc-50 dark:bg-zinc-900 border-b border-zinc-100 dark:border-zinc-800 sticky top-0 z-10">
                  <div class="flex items-center justify-between">
                    <div class="flex items-center gap-1.5">
                      <span class="text-[9px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-500">J{{
                          idx + 1
                        }} · {{ formatDate(day.date) }}</span>
                      <span v-if="getCityNameForDay(day)"
                            class="inline-flex items-center gap-1 text-[8px] font-black text-primary-500 dark:text-primary-400 bg-primary-400/10 px-1.5 py-0.5 rounded-md">
                        <i class="fi fi-rr-building text-[7px]"></i>{{ getCityNameForDay(day) }}
                      </span>
                    </div>
                    <button v-if="!isInvitationPending" @click="focusMapSearchForDay(day.date)"
                            class="text-[9px] font-black text-primary-500 dark:text-primary-400 hover:underline cursor-pointer">
                      + Ajouter
                    </button>
                  </div>
                </div>
                <div class="px-3 py-2 space-y-1.5">
                  <div v-for="stop in day.activities" :key="stop.id">
                    <div @click="focusStopOnMap(stop)"
                      ...
                      <div class="flex-grow min-w-0">
                        <p class="text-[11px] font-black truncate text-zinc-900 dark:text-zinc-100 leading-tight">
                          {{ stop.displayTitle || stop.title }}</p>
                        <p class="text-[9px] text-zinc-400 dark:text-zinc-500 leading-tight">
                          {{ formatStopType(stop.type) }}<span
                            v-if="formatTime(stop.arrivalDate)"> · {{ formatTime(stop.arrivalDate) }}</span>
                        </p>
                      </div>
                      <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all shrink-0">
                        <button v-if="!stop.isReturnStep && !stop.isMorningDeparture && !stop.isEveningReturn"
                                @click.stop="openEditStopModal(stop); showItineraryDrawer = false"
                                class="w-5 h-5 rounded flex items-center justify-center text-zinc-400 hover:text-primary-400 cursor-pointer">
                          <i class="fi fi-rr-edit text-[9px]"></i></button>
                        <button v-if="!stop.isAccommodationHub && !stop.isReturnStep"
                                @click.stop="openDeleteModal('stop', stop)"
                                class="w-5 h-5 rounded flex items-center justify-center text-zinc-400 hover:text-rose-400 cursor-pointer">
                          <i class="fi fi-rr-trash text-[9px]"></i></button>
                      </div>
                    </div>
                    <div v-if="stop.travelTimeToNext && day.activities.indexOf(stop) < day.activities.length - 1"
                         class="flex items-center gap-1.5 pl-4 py-0.5">
                      ...
                      <span class="text-[8px] text-zinc-400 dark:text-zinc-500">{{
                          formatDuration(stop.travelTimeToNext)
                        }} · {{ stop.distanceToNext }}km</span>
                      <span v-if="stop.fuelCostNext" class="text-[8px] text-amber-500/80 flex items-center gap-0.5">· <i
                          class="fi fi-rr-gas-pump"></i>{{ stop.fuelCostNext.toFixed(1) }}€</span>
                      <span v-if="stop.tollCostNext"
                            class="text-[8px] text-violet-500/80 flex items-center gap-0.5">· <i
                          class="fi fi-rr-road"></i>{{ stop.tollCostNext.toFixed(1) }}€</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <main data-onboarding="dash-map"
              class="flex-grow relative bg-zinc-100 dark:bg-[#0c0c0e] overflow-hidden rounded-[2.5rem] my-3 mr-3 border border-zinc-200 dark:border-zinc-800/50"
              :class="{'ml-0': showItineraryDrawer || showBudgetDrawer}">
          ...
          <div class="absolute top-6 left-6 w-full max-w-[420px] z-10 pointer-events-none" ref="mapHeaderRef">
            <div class="relative pointer-events-auto flex items-center">
              <input id="map-search-input" v-model="mapSearchQuery" type="text" @focus="showQuickSuggestions = true"
                     class="w-full bg-white/95 dark:bg-[#1C1C1E]/95 backdrop-blur-2xl border border-zinc-200 dark:border-zinc-700 rounded-2xl pl-12 pr-10 py-4 text-sm font-bold shadow-[0_20px_50px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.5)] focus:ring-2 focus:ring-primary-400/20 focus:border-primary-400 transition-all outline-none text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400 dark:placeholder:text-zinc-500"
                     placeholder="Rechercher un lieu ou une adresse..."/>
              <span
                  class="absolute left-4 flex items-center text-primary-500 dark:text-primary-400 pointer-events-none z-20"><i
                  class="fi fi-rr-search text-xl"></i></span>
              <button v-if="mapSearchQuery" @click="clearMapSearch"
                      class="absolute right-4 flex items-center text-zinc-400 dark:text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-300 cursor-pointer transition-colors z-20">
                <i class="fi fi-rr-cross-small text-xl"></i></button>
              <transition name="fade">
                <div v-if="showQuickSuggestions && !mapSearchQuery"
                     class="absolute top-full mt-2 w-full bg-white/95 dark:bg-[#1C1C1E]/95 backdrop-blur-2xl border border-zinc-200 dark:border-zinc-700 rounded-xl p-2 shadow-2xl">
                  <p class="text-[8px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-500 mb-2 px-1">
                    Suggestions</p>
                  <div class="grid grid-cols-5 gap-1">
                    <button v-for="s in quickSuggestions" :key="s.label" @click="applyQuickSearch(s.label)"
                            class="flex flex-col items-center gap-1 py-2 rounded-lg bg-zinc-50 dark:bg-white/5 hover:bg-primary-400/10 hover:text-primary-500 dark:hover:text-primary-400 text-zinc-500 dark:text-zinc-400 transition-all cursor-pointer">
                      <i :class="s.icon" class="text-xs"></i><span class="text-[7px] font-black">{{ s.label }}</span>
                    </button>
                  </div>
                </div>
              </transition>
          ...
          <div class="absolute top-6 right-6 flex flex-col gap-2 z-10">
            <button @click="fitBounds()"
                    class="w-11 h-11 rounded-2xl bg-white/90 dark:bg-[#1C1C1E]/90 backdrop-blur-xl border border-zinc-200 dark:border-zinc-700 shadow-xl dark:shadow-2xl flex items-center justify-center text-zinc-500 dark:text-zinc-400 hover:text-primary-500 dark:hover:text-primary-400 transition-all cursor-pointer hover:border-primary-400/30">
              <i class="fi fi-rr-location-crosshairs text-base"></i>
            </button>
          </div>

          <div
              class="absolute bottom-6 left-1/2 -translate-x-1/2 z-10 flex items-center gap-2 p-1.5 bg-white/70 dark:bg-[#1C1C1E]/95 backdrop-blur-xl dark:backdrop-blur-2xl border border-zinc-200/50 dark:border-zinc-700/50 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
            <button @click="routeFilters.first = !routeFilters.first"
                    :class="[routeFilters.first ? 'bg-[#ff9900]/10 border-[#ff9900]/30 text-[#ff9900]' : 'bg-zinc-50 dark:bg-white/5 border-transparent text-zinc-400 dark:text-zinc-500 opacity-50']"
                    class="px-3 py-2 rounded-xl border transition-all flex items-center gap-2 cursor-pointer">
              <div class="w-3 h-1 rounded-full bg-[#ff9900]"></div>
              <span class="text-[10px] font-black uppercase tracking-widest">Départ</span>
            </button>
            <button @click="routeFilters.main = !routeFilters.main"
                    :class="[routeFilters.main ? 'bg-[#ccff00]/10 border-[#ccff00]/30 text-[#ccff00]' : 'bg-zinc-50 dark:bg-white/5 border-transparent text-zinc-400 dark:text-zinc-500 opacity-50']"
                    class="px-3 py-2 rounded-xl border transition-all flex items-center gap-2 cursor-pointer">
              <div class="w-3 h-1 rounded-full bg-[#ccff00]"></div>
              <span class="text-[10px] font-black uppercase tracking-widest">Trajets</span>
            </button>
            <button @click="routeFilters.activity = !routeFilters.activity"
                    :class="[routeFilters.activity ? 'bg-[#ff00ff]/10 border-[#ff00ff]/30 text-[#ff00ff]' : 'bg-zinc-50 dark:bg-white/5 border-transparent text-zinc-400 dark:text-zinc-500 opacity-50']"
                    class="px-3 py-2 rounded-xl border transition-all flex items-center gap-2 cursor-pointer">
              <div class="w-3 h-1 rounded-full bg-[#ff00ff]"></div>
              <span class="text-[10px] font-black uppercase tracking-widest">Activités</span>
            </button>
            <button @click="routeFilters.return = !routeFilters.return"
                    :class="[routeFilters.return ? 'bg-[#00ffff]/10 border-[#00ffff]/30 text-[#00ffff]' : 'bg-zinc-50 dark:bg-white/5 border-transparent text-zinc-400 dark:text-zinc-500 opacity-50']"
                    class="px-3 py-2 rounded-xl border transition-all flex items-center gap-2 cursor-pointer">
        ...
        <transition name="panel-slide-right">
          <div v-if="showBudgetDrawer"
               class="w-[380px] shrink-0 flex flex-col bg-white dark:bg-zinc-900 rounded-[1.5rem] border border-zinc-200 dark:border-white/5 m-3 mr-0 overflow-hidden">
            <header
                class="px-4 py-3 border-b border-zinc-100 dark:border-zinc-800/60 flex items-center justify-between shrink-0">
              <div class="flex items-center gap-2.5">
                <div
                    class="w-7 h-7 rounded-lg bg-primary-400/10 flex items-center justify-center text-primary-500 dark:text-primary-400 shrink-0">
                  <i class="fi fi-rr-wallet text-xs"></i></div>
                <div>
                  <h2 class="text-sm font-black tracking-tight text-zinc-900 dark:text-zinc-100 leading-tight">Finances
                    &amp; Budget</h2>
                  <p class="text-[9px] text-zinc-400 dark:text-zinc-500 font-bold">Gestion des dépenses</p>
                </div>
              </div>
              <button @click="showBudgetDrawer = false"
                      class="w-7 h-7 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center justify-center text-zinc-400 dark:text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 transition-all cursor-pointer shrink-0">
                <i class="fi fi-rr-cross text-xs"></i></button>
            </header>
            <div class="px-4 py-3 border-b border-zinc-100 dark:border-zinc-800/40 shrink-0">
              <div class="flex items-center justify-between mb-2">
                <div>
                  <p class="text-[8px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-500 mb-0.5">
                    Dépensé / Budget</p>
                ...
                <div class="text-right">
                  <p class="text-[8px] font-black uppercase tracking-widest text-primary-500 dark:text-primary-400 mb-0.5">
                    Restant</p>
                  <p class="text-lg font-black text-primary-500 dark:text-primary-400">
                    {{ Math.round(budgetStats.remaining) }}€</p>
                </div>
              </div>
              ...
              <div class="flex items-center gap-4 mt-2">
                <span class="text-[8px] font-black text-amber-500/80 flex items-center gap-1"><i
                    class="fi fi-rr-gas-pump"></i>Essence ~{{ Math.round(estimatedFuelCost) }}€</span>
                <span class="text-[8px] font-black text-violet-500/80 flex items-center gap-1"><i
                    class="fi fi-rr-road"></i>Péages ~{{ Math.round(estimatedTollCost) }}€</span>
              </div>
              <div v-if="trip.status === 'completed'" class="mt-4">
                <button @click="showSettlementModal = true"
                        class="w-full py-2.5 bg-violet-600 hover:bg-violet-700 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all cursor-pointer flex items-center justify-center gap-2 shadow-lg shadow-violet-600/20 border-none">
            ...
            <div class="px-4 py-2.5 shrink-0 border-b border-zinc-100 dark:border-zinc-800/40">
              <button @click="showExpenseModal = true"
                      class="w-full py-2 rounded-xl bg-primary-400 text-zinc-900 text-[10px] font-black uppercase tracking-widest hover:bg-primary-500 transition-all cursor-pointer flex items-center justify-center gap-1.5">
            ...
            <div class="flex-grow overflow-y-auto custom-scrollbar">
              <div v-if="!trip.expenses?.length" class="py-12 text-center">
                ...
                <p class="text-[9px] text-zinc-400 dark:text-zinc-600 uppercase tracking-widest font-black">Aucune
                  dépense</p>
              </div>
              <div class="px-3 py-2 space-y-1.5">
                <div v-for="exp in trip.expenses" :key="exp.id"
                  ...
                  <div
                      class="w-7 h-7 rounded-lg bg-primary-400/10 text-primary-500 dark:text-primary-400 flex items-center justify-center shrink-0">
                  ...
                  <div class="flex-grow min-w-0">
                    ...
                    <p class="text-[9px] text-zinc-400 dark:text-zinc-500 leading-tight">{{
                      ...
                      }} · {{ exp.payer?.fullName || '?' }}</p>
                  </div>
                  <div class="flex items-center gap-1.5 shrink-0">
                    <span class="text-xs font-black text-rose-500">-{{ exp.amount }}€</span>
                    <button @click.stop="openEditExpenseModal(exp)"
                            class="opacity-0 group-hover:opacity-100 w-5 h-5 rounded flex items-center justify-center text-zinc-400 hover:text-primary-400 cursor-pointer transition-all">
                      <i class="fi fi-rr-edit text-[9px]"></i></button>
                    <button @click.stop="openDeleteModal('expense', exp)"
                            class="opacity-0 group-hover:opacity-100 w-5 h-5 rounded flex items-center justify-center text-zinc-400 hover:text-rose-400 cursor-pointer transition-all">
                      <i class="fi fi-rr-trash text-[9px]"></i></button>
                  </div>
                </div>
              </div>
      ...
      <div v-if="showExpenseModal"
        ...
        <div
            class="w-full max-w-md bg-white dark:bg-[#111113] rounded-[2rem] border border-zinc-200/80 dark:border-white/8 shadow-[0_32px_80px_rgba(0,0,0,0.18)] dark:shadow-[0_32px_80px_rgba(0,0,0,0.7)] overflow-hidden animate-slide-up">
          <div class="h-1 w-full bg-primary-400"></div>
          <div class="px-6 py-5 flex items-center justify-between">
            <div class="flex items-center gap-3.5">
              <div
                  class="w-10 h-10 rounded-2xl bg-primary-400/10 flex items-center justify-center text-primary-500 dark:text-primary-400 shrink-0">
                <i class="fi fi-rr-receipt text-base leading-none"></i></div>
              <div>
                <h3 class="font-black text-base text-zinc-900 dark:text-zinc-100 tracking-tight leading-tight">Ajouter
                  une dépense</h3>
                <p class="text-xs text-zinc-400 dark:text-zinc-500 mt-0.5">Saisir une nouvelle dépense</p>
              </div>
            </div>
            <button @click="showExpenseModal = false"
                    class="w-8 h-8 rounded-xl flex items-center justify-center text-zinc-400 dark:text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800 cursor-pointer transition-all">
              <i class="fi fi-rr-cross-small text-base leading-none"></i></button>
          </div>
          <form @submit.prevent="createExpense" class="px-6 pb-6 space-y-4">
            <div>
              <label
                  class="text-[10px] font-black uppercase text-zinc-400 dark:text-zinc-500 tracking-widest mb-1.5 block">Titre</label>
              <input v-model="newExpense.title" required
                     class="w-full bg-zinc-100 dark:bg-zinc-800/60 border border-zinc-200 dark:border-zinc-700 focus:border-primary-400 focus:ring-2 focus:ring-primary-400/20 rounded-xl px-4 py-3 text-sm font-bold text-zinc-900 dark:text-zinc-100 outline-none transition-all"
                     placeholder="Ex: Plein d'essence"/>
            </div>
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label
                    class="text-[10px] font-black uppercase text-zinc-400 dark:text-zinc-500 tracking-widest mb-1.5 block">Montant
                  (€)</label>
              ...
              <div>
                <label
                    class="text-[10px] font-black uppercase text-zinc-400 dark:text-zinc-500 tracking-widest mb-1.5 block">Catégorie</label>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label
                    class="text-[10px] font-black uppercase text-zinc-400 dark:text-zinc-500 tracking-widest mb-1.5 block">Payé
                  par</label>
              ...
              <div>
                <label
                    class="text-[10px] font-black uppercase text-zinc-400 dark:text-zinc-500 tracking-widest mb-1.5 block">Date</label>
            ...
            <div class="flex gap-3 pt-1">
              ...
              <button type="submit" :disabled="isSubmitting"
                      class="flex-[2] bg-primary-400 hover:bg-primary-500 text-zinc-900 font-black text-sm rounded-xl py-3 transition-all shadow-lg shadow-primary-400/25 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50">
      ...
      <div v-if="showEditExpenseModal && editingExpense"
           class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-zinc-950/60 backdrop-blur-md"
        ...
        <div
          ...
          <div class="px-6 py-5 flex items-center justify-between">
            <div class="flex items-center gap-3.5">
              <div
                  class="w-10 h-10 rounded-2xl bg-amber-400/10 flex items-center justify-center text-amber-500 dark:text-amber-400 shrink-0">
                <i class="fi fi-rr-edit text-base leading-none"></i></div>
              <div>
                <h3 class="font-black text-base text-zinc-900 dark:text-zinc-100 tracking-tight leading-tight">Modifier
                  la dépense</h3>
                <p class="text-xs text-zinc-400 dark:text-zinc-500 mt-0.5 truncate max-w-[200px]">
                  {{ editingExpense.title }}</p>
              </div>
            </div>
            <button @click="showEditExpenseModal = false"
                    class="w-8 h-8 rounded-xl flex items-center justify-center text-zinc-400 dark:text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800 cursor-pointer transition-all">
              <i class="fi fi-rr-cross-small text-base leading-none"></i></button>
          </div>
          <form @submit.prevent="updateExpense" class="px-6 pb-6 space-y-4">
            <div>
              ...
              <input v-model="editingExpense.title" required
                     class="w-full bg-zinc-100 dark:bg-zinc-800/60 border border-zinc-200 dark:border-zinc-700 focus:border-amber-400 focus:ring-2 focus:ring-amber-400/20 rounded-xl px-4 py-3 text-sm font-bold text-zinc-900 dark:text-zinc-100 outline-none transition-all"/>
            </div>
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label
            ...
            <div class="flex gap-3 pt-1">
              ...
              <button type="submit" :disabled="isSubmitting"
                      class="flex-[2] bg-amber-400 hover:bg-amber-500 text-zinc-900 font-black text-sm rounded-xl py-3 transition-all shadow-lg shadow-amber-400/25 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50">
                <span v-if="isSubmitting" class="spinner w-4 h-4"></span>
      ...
      <TripEditModal v-if="trip && showEditModal" :trip="trip" :isSubmitting="isSubmitting"
                     @close="showEditModal = false" @update="handleUpdateTrip"/>
      <TripPhotosModal v-if="showPhotosModal && selectedStopForPhotos" :stop="selectedStopForPhotos"
                       @close="showPhotosModal = false; selectedStopForPhotos = null"/>
      <TripAddStopModal v-if="showAddStopModal && trip"
                        ...
                        @added="fetchTripData"/>
      <AppModal v-model="showDeleteConfirmModal" type="danger"
                :title="itemToDelete?.action === 'leave' ? 'Quitter le voyage' : 'Supprimer'"
                :message="itemToDelete?.action === 'leave' ? 'Êtes-vous sûr de vouloir quitter ce voyage ?' : `Supprimer '${itemToDelete?.name}' ?`"
                confirm-text="Confirmer" cancel-text="Annuler" @confirm="confirmDeleteItem"/>
      ...
      <Teleport to="body">
        <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 scale-95"
          ...
          <div v-if="showSettlementModal"
               class="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-zinc-950/60 backdrop-blur-md"
               @click.self="showSettlementModal = false">
            <div
                class="w-full max-w-2xl bg-white dark:bg-[#111113] rounded-[2.5rem] border border-zinc-200/80 dark:border-white/8 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
              <div class="h-1.5 w-full bg-violet-500"></div>
              <div
                  class="px-8 py-6 flex items-center justify-between border-b border-zinc-100 dark:border-white/5 shrink-0">
                <div class="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p class="text-[10px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-500">
                      Répartition finale des dépenses</p>
                  </div>
                </div>
                <button @click="showSettlementModal = false"
                        class="w-10 h-10 rounded-xl flex items-center justify-center text-zinc-400 hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-white/5 transition-all cursor-pointer border-none bg-transparent">
              ...
              <div class="flex-grow overflow-y-auto p-8 custom-scrollbar">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                  <div class="p-5 rounded-3xl bg-zinc-50 dark:bg-white/5 border border-zinc-100 dark:border-white/5">
                    <p class="text-[9px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-500 mb-1">
                      Dépenses directes</p>
                    <p class="text-xl font-black text-zinc-900 dark:text-white">{{
                        Math.round(settlementData.expenses)
                      }}€</p>
                  </div>
                  <div class="p-5 rounded-3xl bg-zinc-50 dark:bg-white/5 border border-zinc-100 dark:border-white/5">
                    <p class="text-[9px] font-black uppercase tracking-widest text-amber-500 mb-1">Carburant
                      (estimé)</p>
                ...
                <div class="space-y-3">
                  <div v-for="p in settlementData.participants" :key="p.id"
                    ...
                    <div
                        class="w-12 h-12 rounded-2xl bg-zinc-100 dark:bg-white/5 flex items-center justify-center text-xs font-black uppercase shrink-0">
                      <img v-if="p.avatar" :src="getAvatarUrl(p.avatar)"
                    ...
                    <div class="flex-grow min-w-0">
                      ...
                      <p class="text-[10px] text-zinc-500 dark:text-zinc-400">Déjà payé : {{
                          Math.round(p.alreadyPaid)
                        }}€</p>
                    </div>
                    <div class="text-right shrink-0">
                      ...
                      <div v-else>
                        ...
                        <input type="number" v-model="manualShares[p.id]"
                               class="w-20 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-2 py-1 text-right text-xs font-bold text-zinc-900 dark:text-white outline-none focus:border-violet-500"/>
                      </div>
                    </div>
                    <div class="w-32 text-right shrink-0 px-4 border-l border-zinc-100 dark:border-white/5">
                      <p class="text-[9px] font-black uppercase tracking-widest mb-0.5"
                         :class="p.balance > 0 ? 'text-rose-500' : 'text-primary-500'">
              ...
              <div
                  class="p-8 bg-zinc-50 dark:bg-white/2 shrink-0 flex items-center justify-between border-t border-zinc-100 dark:border-white/5">
                <div class="flex items-center gap-6">
                  <div>
                    <p class="text-[8px] font-black uppercase tracking-widest text-zinc-400 mb-0.5">Total Voyage</p>
                    <p class="text-xl font-black text-zinc-900 dark:text-white">{{
                        Math.round(settlementData.total)
                      }}€</p>
                  </div>
                  <div v-if="settlementMode === 'manual'">
                    ...
                    <p class="text-xl font-black"
                       :class="Math.round(Object.values(manualShares).reduce((a, b) => a + b, 0)) === Math.round(settlementData.total) ? 'text-primary-500' : 'text-rose-500'">
                      {{ Math.round(Object.values(manualShares).reduce((a, b) => a + b, 0)) }}€
                    </p>
                  </div>
                </div>
                <button @click="showSettlementModal = false"
                        class="px-8 py-3 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 text-xs font-black uppercase tracking-widest rounded-2xl hover:opacity-90 transition-all cursor-pointer border-none">
                  Fermer le bilan
                </button>
              </div>
            </div>
      ...
      <Teleport to="body">
        <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0"
          ...
          <div v-if="showPublicModal"
               class="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-zinc-950/60 backdrop-blur-md"
               @click.self="showPublicModal = false">
            <div
                class="w-full max-w-md bg-white dark:bg-[#111113] rounded-[2.5rem] border border-zinc-200/80 dark:border-white/8 shadow-2xl overflow-hidden animate-slide-up">
              <div class="p-8 text-center">
                <div
                    class="w-20 h-20 bg-violet-500/10 rounded-3xl flex items-center justify-center mx-auto mb-6 text-violet-500">
                ...
                <p class="text-zinc-500 dark:text-zinc-400 text-sm mb-8">Choisissez une catégorie pour que la communauté
                  puisse découvrir votre voyage.</p>
                ...
                <div class="flex gap-3">
                  ...
                  <button @click="makeTripPublic" :disabled="isSubmitting || !selectedCategory"
...
.pac-item > span:not(.pac-item-query):not(.pac-icon):empty {
  display: none !important;
}

.pac-item:has(span:not(.pac-item-query):not(.pac-icon):empty) .pac-item-query {
