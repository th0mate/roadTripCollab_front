...
onMounted(async () => {
  ...
  try {
    ...
    success.value = true; message.value = response.data.message
  } catch (err: any) {
    ...
    <div class="w-full max-w-sm">
      <div class="paper-card p-8 text-center">
        ...
        <div v-else-if="success" class="flex flex-col items-center gap-4 py-4">
          ...
          <div>
            <h1 class="text-xl font-bold text-zinc-900 dark:text-white mb-1">Email vérifié !</h1>
            <p class="text-sm text-zinc-500 dark:text-zinc-400">{{ message }}</p>
          </div>
          <RouterLink to="/login" class="btn-primary w-full justify-center py-2.5">
            Se connecter
            <i class="fi fi-rr-arrow-right text-sm leading-none"></i>
          </RouterLink>
        </div>
        ...
        <div v-else class="flex flex-col items-center gap-4 py-4">
          ...
          <RouterLink to="/login" class="btn-secondary w-full justify-center py-2.5">
